<!--
  centurio1987 프로필 README
  본문은 `~/.claude/skills/profile-update` 스킬이 마커 영역만 자동 갱신합니다.
  마커 밖(인사말, Connect, GitHub Stats 등)은 사람이 직접 편집합니다.
-->

### 👋 안녕하세요, 김윤덕입니다

<!-- profile:headline:start -->
**Product Owner · Node.js Engineer · CTO · Product Engineer** — 경력 6년 3개월
<!-- profile:headline:end -->

<!-- profile:intro:start -->
자원이 부족하고 방향이 자주 바뀌는 스타트업에서 **0→1 제품과 팀의 개발 시스템을 함께 설계**해 왔습니다.
PO로 전략·제품을 그리고, CTO로 의사결정의 기술 정합성을 책임지며,
**NestJS로 도메인을 직접 손코딩하는 Product Engineer**입니다.
B2B SaaS와 DX 솔루션을 0→1로 만들고, 온프레미스 네이티브 Kubernetes 위에 제품팀 시스템을 정착시켰습니다.
한 사람이 **기획에서 구현까지 all-in-one**으로 책임지는 흐름을 좋아합니다.
<!-- profile:intro:end -->

<!-- profile:bullets:start -->
- 🔭 현재 관심사: Plugin 아키텍처 운영 · 디자인 시스템 · 블로그/포트폴리오 콘텐츠·SEO 개선 · 온프레미스 네이티브 K8s
- 🌱 학습 중: Rust (`estd` 실험), 알고리즘/자료구조 가이드(grow-up-programming), 개인 지식 베이스(my-wiki)
- 💼 **PO · CTO · Product Engineer 역할을 한 흐름으로 풀 수 있는 자리를 찾고 있습니다 (Open to opportunities)**
- 📫 연락: centurio87@naver.com
<!-- profile:bullets:end -->

---

#### 🧩 무엇을 해왔나

<!-- profile:experience-summary:start -->
- **풀스택 (Node.js 중심)** — NestJS · GraphQL · TypeORM/Prisma · PostgreSQL 기반 도메인 모델링과 API 운영. DDD·레이어드/헥사고날과 Plugin 아키텍처로 0→1을 짠다.
- **프론트엔드 풀스택** — Vue/Nuxt·Next·TypeScript. Atomic Design 기반 컴포넌트 분리, Service Worker+IndexedDB 캐시 동기화.
- **인프라·DevOps** — On-Premise 네이티브 K8s(Talos OS·iPXE) · Terraform/Terramate IaC · Keycloak SSO/IAM · GitOps · 관측가능성 · ISO 27001·27017.
- **제품·PO** — 전략 분석(PESTEL·3C·5Forces·STP) · Contextual Inquiry 기반 제품 발견 · PMF 정렬 제품 정의·제품화 전략.
- **리더십·팀 시스템** — IT 거버넌스·개발 표준 수립, 팀원을 Product Engineer로 육성해 기획~엔지니어링을 한 사람이 책임지게 한다.
- **도메인** — B2B SaaS·DX 솔루션·고가 시계 시세/거래·앱 푸시/메트릭 수집·On-Premise Cloud 솔루션.
<!-- profile:experience-summary:end -->

#### 🔬 사이드 프로젝트

<!-- profile:projects:start -->
> 2025년 이후 시작한 작업 위주로 정리했습니다. 공개 repo는 직접 들어가서 보실 수 있고,
> 일부 작업은 공개 가능한 범위의 이름과 요약만 표기합니다.

**공개**

| 프로젝트 | 설명 |
|---|---|
| [bbangto-ui](https://github.com/centurio1987/bbangto-ui) | TypeScript UI 패키지 — 스타일 가이드 카탈로그와 variant/layout 프리셋을 제공하는 실사용 프로젝트 |
| [mindful-fortune-teller](https://github.com/centurio1987/mindful-fortune-teller) | TypeScript 마음챙김 포춘텔러 앱 — 매일의 운세와 대화형 경험을 제공하는 프로젝트 |
| [centurio1987.github.io](https://github.com/centurio1987/centurio1987.github.io) | MDX 포트폴리오·블로그 — SEO, 이미지 최적화, 글쓰기 워크플로를 함께 개선하는 개인 사이트 |
| [tech-news-wiki](https://github.com/centurio1987/tech-news-wiki) | Python 테크 뉴스 위키 — 오늘의 기술 뉴스를 수집해 독자에게 큐레이션하는 파이프라인 |
| [my-claude-plugins](https://github.com/centurio1987/my-claude-plugins) | 개인 Claude 플러그인 마켓플레이스 — 웹 리서치·MDX 다이어그램·wiki-forge 자동화 실험 |
| [grow-up-programming](https://github.com/centurio1987/grow-up-programming) | 알고리즘·자료구조 학습 가이드 — MDX 가이드와 실습 스텁 기반 학습 환경 |
| [my-wiki](https://github.com/centurio1987/my-wiki) | 개인 지식 베이스 위키 |
| [design-pattern-guide](https://github.com/centurio1987/design-pattern-guide) | 디자인 패턴 학습·정리 노트 |
| [my-nvim](https://github.com/centurio1987/my-nvim) | 개인 Neovim 설정 |

**비공개 (요약만)**

- `stock-analysis` — 주식 투자 인사이트 보고 자동 생성
- `tr-docs` — 공식 문서·PDF를 EPUB으로 재구성하는 도구 (Python)
- `estd` — Rust 학습/실험
- `infrastructure` — 개인 인프라 IaC (HCL)
- `obsidian-writing-vault` — 개인 글쓰기 vault
<!-- profile:projects:end -->

---

#### 🛠 기술 스택

**주력**

<!-- profile:tech-primary:start -->
![TypeScript](https://img.shields.io/badge/typescript-3178C6?logo=typescript&logoColor=white)
![NestJS](https://img.shields.io/badge/nestjs-E0234E?logo=nestjs&logoColor=white)
![Vue](https://img.shields.io/badge/vue-4FC08D?logo=vue.js&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/postgreSQL-4169E1?logo=postgresql&logoColor=white)
![Kubernetes](https://img.shields.io/badge/kubernetes-326CE5?logo=kubernetes&logoColor=white)
![Terraform](https://img.shields.io/badge/terraform-844FBA?logo=terraform&logoColor=white)
<!-- profile:tech-primary:end -->

**사용 경험**

<!-- profile:tech-secondary:start -->
![Nuxt](https://img.shields.io/badge/nuxt-00DC82?logo=nuxt.js&logoColor=white)
![Next.js](https://img.shields.io/badge/next.js-000000?logo=nextdotjs&logoColor=white)
![GraphQL](https://img.shields.io/badge/graphql-E10098?logo=graphql&logoColor=white)
![Prisma](https://img.shields.io/badge/prisma-2D3748?logo=prisma&logoColor=white)
![TypeORM](https://img.shields.io/badge/typeorm-FE0803?logo=typeorm&logoColor=white)
![MongoDB](https://img.shields.io/badge/mongoDB-47A248?logo=mongodb&logoColor=white)
![Terramate](https://img.shields.io/badge/terramate-000000?logoColor=white)
![Rust](https://img.shields.io/badge/rust-000000?logo=rust&logoColor=white)
<!-- profile:tech-secondary:end -->

---

#### 🔗 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/yoondeok-kim-319bb8145)
[![Portfolio](https://img.shields.io/badge/Portfolio-181717?logo=githubpages&logoColor=white)](https://centurio1987.github.io)

---

<details>
<summary>📊 GitHub Stats</summary>

<br/>

[![GitHub Profile Stats](./profile/stats.svg)](./profile/stats.svg)
[![Top Langs](./profile/top-langs.svg)](./profile/top-langs.svg)
[![GitHub Streak](https://streak-stats.demolab.com?user=centurio1987)](https://git.io/streak-stats)

</details>

<!-- profile:last-updated:start -->
<!-- Last updated: 2026-07-10 -->
<!-- profile:last-updated:end -->
